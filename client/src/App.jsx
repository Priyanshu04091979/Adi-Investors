  ...
  return (
    <BrowserRouter>
      ...
      <div className="flex flex-col min-h-screen bg-white text-ink-dark">
        <motion.div 
          className="fixed top-0 left-0 right-0 h-1 bg-gradient-to-r from-gold-400 to-[#855632] z-[10000] origin-[0%]"
          style={{ scaleX }}
        />
        <Navbar />
            ...
            <AnimatePresence>
              {!isChatOpen && showTooltip && (
                <motion.div
                  ...
                  className="absolute bottom-[4px] right-0 bg-[#422522] text-white p-4 rounded-2xl shadow-[0_10px_25px_rgba(66,37,34,0.2)] border border-[#855632] w-64 flex flex-col items-start gap-1 text-sm font-sans"
                  style={{ transformOrigin: "bottom right" }}
                >
                  {/* Close button */}
                  <button 
                    onClick={() => setShowTooltip(false)}
                    className="absolute top-2.5 right-2.5 text-[#FAF6F0]/70 hover:text-[#FAF6F0] transition-colors"
                  >
                    <X size={14} />
                  </button>
                  ...
                  <div className="absolute right-6 -bottom-1.5 w-3 h-3 bg-[#422522] border-r border-b border-[#855632] rotate-45" />
                </motion.div>
              )}
            </AnimatePresence>
          </div>
