...
export default function ChatMessage({ role, content, timestamp }: ChatMessageProps) {
  ...
  return (
    ...
    >
      {!isUser && (
        <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#BD8A53] to-[#A1713B] flex items-center justify-center shrink-0 mr-3 mt-1 shadow-sm border border-[#FAF6F0]/35">
      ...
      <div className={`flex flex-col ${isUser ? 'items-end' : 'items-start'} max-w-[85%]`}>
        <div
          className={`px-5 py-3.5 rounded-3xl text-[15px] leading-relaxed shadow-sm ${
            isUser
               ? 'bg-[#855632] text-white rounded-tr-sm'
