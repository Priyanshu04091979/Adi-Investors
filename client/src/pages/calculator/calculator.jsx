...
function Calculator() {
  ...
  const calculateReturns = () => {
    ...
    let calcMode = '';
    if (activeCalc === 'SIP Calculator' || activeCalc === 'RD Calculator') {
      calcMode = 'SIP';
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-green-700/5 p-6 sticky top-28">
              ...
              <div className="flex flex-col gap-2">
                {calculatorsList.map((calc, idx) => (
                  <button
                    key={idx}
                    onClick={() => handleCalcChange(calc)}
                    className={`flex items-center justify-between p-3 rounded-lg text-sm font-medium transition-colors ${activeCalc === calc
                        ? 'bg-green-50 text-green-700 border border-green-100'
                        : 'text-gray-600 hover:bg-slate-50 hover:text-green-700 border border-transparent'
                      }`}
                  >
                    <span>{calc}</span>
                    <Icons.ChevronRight size={16} className={activeCalc === calc ? 'text-green-700' : 'text-gray-400'} />
                  </button>
                ))}
              </div>
            </div>
